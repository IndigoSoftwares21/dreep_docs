...
Dreep uses a highly efficient, two-step upload process that allows your users (or your backend) to upload files through a dedicated streaming relay. The relay never buffers the file in memory — bytes are piped straight through to storage as they arrive — so there's no size ceiling like the single-request multipart endpoint has.
...
`filename` is optional — `key` doubles as both the filename and the folder
path, and if you provide neither, Dreep generates a name for you. Naming by
`key` alone (no `filename`):
...
curl -X POST https://api.dreep.cloud/api/v1/upload/presign \
  ...
  -d '{
    "key": "launch/2024/video.mp4",
    "contentType": "video/mp4",
    "sizeBytes": 52428800
  }'
```

Or naming by `filename` with a separate `folder`:
...
The API will return an `uploadUrl` and a tracking `id`. The `uploadUrl` is a
single-use link, valid for 10 minutes — treat it as opaque and don't try to
parse or reuse it.

```json
{
  "id": "2f928a3f-1d2a-4a2b-8a8b-123456789abc",
  "uploadUrl": "https://cdn.dreep.cloud/api/v1/upload/2f928a3f-1d2a-4a2b-8a8b-123456789abc"
}
```
...
upload_url = "https://cdn.dreep.cloud/api/v1/upload/2f928a3f-1d2a-4a2b-8a8b-123456789abc"
file_path = "video.mp4"
mime_type = "video/mp4"
